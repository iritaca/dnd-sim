...
import { generateMonsters } from "../PlayerAndMonsters/monsterCreation";

const RandomStage = (level) => {
  let monsterList = generateMonsters(10);
  let stage = [];
  let round = [];
  let minValue = 1;
  let filterMode;
  ...
  for (let i = 0; i < 3; i++) {
    ...
    while (gamemode[i].gamePoints >= minValue) {
      ...
      if (
        randomMonsterPoints <= gamemode[i].gamePoints &&
        randomMonsterPoints >= 0
      ) {
...
// return {level, stage}
// const a = RandomStage('easy')
// a.level
// a.stage
// const {level, stage} = RandomStage('easy')
